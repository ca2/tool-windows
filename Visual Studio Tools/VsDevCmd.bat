...
@REM [devenv|wdexpress].isolation.ini location: Microsoft Visual Studio 16.0\Common7\IDE
...
if "%VSCMD_BANNER_TEXT_ALT%"=="" (
    set "__VSCMD_BANNER_TEXT=Visual Studio 2019 Developer Command Prompt v%VSCMD_VER%"
) else (
...
if "%VSCMD_ARG_no_logo%"=="" (
    ...
    @echo ** Copyright ^(c^) 2019 Microsoft Corporation
    @echo **********************************************************************
)
