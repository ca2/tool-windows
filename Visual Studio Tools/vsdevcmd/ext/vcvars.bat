...
@echo                             [VSInstallDir]\VC\Auxiliary\Build\Microsoft.VCToolsVersion.v142.default.txt will be used.
@echo            ** 14.0        : VS 2015 (v140) VC++ Toolset (installation of the v140 toolset is a prerequisite)
@echo            ** 14.1x       : VS 2017 (v141) VC++ Toolset, if that version is installed on the system under 
@echo                             [VSInstallDir]\VC\MSVC\Tools\[version].  Where '14.1x' specifies a partial 
@echo                             [version]. The latest [version] directory that matches the specified value will 
@echo                             be used.
@echo            ** 14.1x.yyyyy : VS 2017 (v141) VC++ Toolset, if that version is installed on the system under 
@echo                             [VSInstallDir]\VC\MSVC\Tools\[version]. Where '14.1x.yyyyy' specifies an 
...
@REM We will "fallback" to Microsoft.VCToolsVersion.default.txt (v141) if Microsoft.VCToolsVersion.v142.default.txt does not exist.
if EXIST "%VCINSTALLDIR%Auxiliary\Build\Microsoft.VCToolsVersion.v142.default.txt" (
    if "%VSCMD_DEBUG%" GEQ "2" @echo [DEBUG:%~nx0] Microsoft.VCToolsVersion.v142.default.txt was found.
    set "__VCVARS_DEFAULT_CONFIG_FILE=%VCINSTALLDIR%Auxiliary\Build\Microsoft.VCToolsVersion.v142.default.txt"

) else (
    if "%VSCMD_DEBUG%" GEQ "1" @echo [DEBUG:%~nx0] Microsoft.VCToolsVersion.v142.default.txt was not found. Defaulting to 'Microsoft.VCToolsVersion.default.txt' instead to check for v141.
)
...
@REM We will "fallback" to Microsoft.VCRedistVersion.default.txt (v141) if Microsoft.VCRedistVersion.v142.default.txt does not exist on disk.
if exist "%VCINSTALLDIR%Auxiliary\Build\Microsoft.VCRedistVersion.v142.default.txt" (
    set "__VCVARS_DEFAULT_REDIST_FILE=%VCINSTALLDIR%Auxiliary\Build\Microsoft.VCRedistVersion.v142.default.txt"
    if "%VSCMD_DEBUG%" GEQ "2" @echo [DEBUG:%~nx0] Microsoft.VCRedistVersion.v142.default.txt found.
) else (
    if "%VSCMD_DEBUG%" GEQ "1" @echo [DEBUG:%~nx0] Microsoft.VCRedistVersion.v142.default.txt was not found. Defaulting to 'Microsoft.VCRedistVersion.default.txt' instead to check for v141.
)
